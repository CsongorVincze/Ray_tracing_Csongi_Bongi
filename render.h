...
color ray_color(sphere sp_array[], int num_spheres, ray_3 ray, hit_rec *hitdata, double reflection_number) {// itt mi a tokom van
    ...
    if(which_hit(sp_array, num_spheres, ray, (interval){.min = 0.0, .max = 20.0}, hitdata)){
        ...
        return _mul_s(reflection_number, ray_color(sp_array, num_spheres, new_ray, hitdata, reflection_number)); //todo: itt el kell tudjuk erni azt h milyen vilagosak legyenek a gombok
    }
    ...
    for(int j = 0; j < image_height; j++){
        for(int i = 0; i < image_width; i++){
            ...
            for(int m = 0; m < mosas; m++){
                ...
                pixel_color = _add(pixel_color, ray_color(sp_array, num_spheres, sugarka, &hitdata, reflection_number)); //? itt ez az egesz elmosos folyamat egy kicsi furan mukodik
            }        
